...
if __name__ == "__main__":
    ...
    for nr in draw_nrs:
        for i in range(len(boards)):
            ...
            if board_is_complete(boards_bool[i]) and i not in [place["i"] for place in leaderboard]:
                sum_unmarked = board_sum_unmarkeds(boards[i], boards_bool[i])
                score = int(nr) * sum_unmarked
                prt_str = f"{sum_unmarked} * {int(nr)} = {score}"
                leaderboard.append({"i": i,
                                    "score": score,
                                    "string": prt_str})
            if len(leaderboard) == len(boards):
                print(f"Board {leaderboard[-1]['i']} -> {leaderboard[-1]['string']}")
                quit()
    print(-1)
